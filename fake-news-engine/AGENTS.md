...
For this project, treat the UI as a public-awareness alert surface, not a generic admin dashboard.

- Lead with immediacy: the first screen should communicate what is happening now and why it matters.
- Make credibility signals obvious: source, domain, confidence, and processing status should be easy to scan.
- Prefer a bold editorial layout over generic enterprise chrome; the interface should feel like a live public advisory feed.
- Keep the interface responsive on desktop and mobile, with compact alert cards and clear hierarchy.
- Use real-time subscriptions only; do not introduce polling or fetch-based refreshes.
