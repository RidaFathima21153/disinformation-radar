...
const ArticleMetadata = table({ name: 'article_metadata', public: true }, {
...
const DetectionInference = table({ name: 'detection_inference', public: true }, {
...
const SystemUser = table({ name: 'system_user', public: true }, {
...
export const ingest_news_article = fakeNewsSchema.reducer({
    ...
    if (ctx.db.ArticleMetadata.source_url.find(source_url))
        return;
...
export const finalize_inference = fakeNewsSchema.reducer({
    ...
    if (!record)
        throw new SenderError(`No inference record for article_id ${article_id}.`);
...
export const trigger_nlp_analysis = fakeNewsSchema.procedure({ article_id: t.u64(), text_payload: t.string() }, t.unit(), (ctx, { article_id, text_payload }) => {
    try {
